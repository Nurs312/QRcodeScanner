...
import insertPackageRecord from '@salesforce/apex/barcodeScannerController.insertPackageRecord';
import setStatusToDelivered from '@salesforce/apex/barcodeScannerController.setStatusToDelivered';
...
export default class BarcodeScanner extends LightningElement {
    ...
    sendBarcodeValues(){
        insertPackageRecord({payload: JSON.stringify(this.scannedBarcode)})
        .then((result)=>console.log(result))
        .catch((error)=>console.log(error))
    }

    sendBarcodeValuesForUpdate(){
        setStatusToDelivered({payload: JSON.stringify(this.deliveredPackage)})
        .then((result)=>console.log(result))
        .catch((error)=>console.log(error))
    }
    ...
    // Handler for scanning     qr code to set package's status to "Delivered"
    handleMarkAsDelivered(event) {
        // Reset deliveredPackage to empty string before starting new scan
        ...
        if (this.myScanner != null && this.myScanner.isAvailable()) {
            ...
            this.myScanner
                ...
                .then((result) => {
                    ...
                    this.deliveredPackage = JSON.parse(result.value);
                    console.log(this.deliveredPackage);
                    this.sendBarcodeValuesForUpdate();
                    this.dispatchEvent(
                        new ShowToastEvent({
        ...
        if (this.myScanner != null && this.myScanner.isAvailable()) {
            ...
            this.myScanner
                ...
                .then((result) => {
                    ...
                    // - and so on; this is YOUR code
                    // Here, we just display the scanned value in the UI
                    this.scannedBarcode = JSON.parse(result.value);
                    console.log(this.scannedBarcode);
                    this.sendBarcodeValues();
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Successful Scan',
                            message: 'Barcode scanned successfully.',
